...
int main(int argc, char *argv[])
{
	...
	if (pthread_getschedparam(pthread_self(),&sched_type, &threadparam) != EOK){
		slogf(_SLOG_SETCODE(_SLOGC_CHAR, 0), _SLOG_WARNING,"WDT:  get priority request failed");
		return EXIT_FAILURE;
	}
	if (priority != threadparam.sched_priority){
		...
		if (pthread_setschedparam(pthread_self(),sched_type, &threadparam) != EOK) {
			slogf(_SLOG_SETCODE(_SLOGC_CHAR, 0), _SLOG_WARNING,"WDT:  can't change priority");
		}
	}
...
__SRCVERSION("$URL: http://svn.ott.qnx.com/product/branches/7.0.0/trunk/hardware/support/wdtkick/main.c $ $Rev: 808714 $")
#endif
