                       ...
                       uint32_t offset)
{
	...
	*dbase=virtual[0];
	...
	/* weilun@adv - start */
	/* support rom5420 a1 or b1, a1 chip did = 0xba, b1 chip id = 0xbb */
	if (did == 0xbb)
		dbase->jedec_lo = 0xbb;
	/* weilun@adv - end */
	...
	if (mid != dbase->jedec_hi || did != dbase->jedec_lo || u_size * dbase->geo_vect[0].unit_num != size ) {
		printf("weilun@adv fs0 read_ident mid=0x%x, did=0x%x doesn't match database\n", mid, did);
		return (ENOENT);
	}
...
__SRCVERSION("$URL: http://svn.ott.qnx.com/product/branches/7.0.0/trunk/hardware/flash/boards/norspi/f3s_spi_ident.c $ $Rev: 680332 $")
#endif
