			...
			int ops)
{
	...
	if (verbose > 2) fprintf(stderr, "%s: lock status %"PRIx64" \n", __func__, status);
	if (ops == LOCK_GET) {
		if ((status & mask) == mask) {
			ret = 0;
		} else {
...
__SRCVERSION("$URL: http://svn.ott.qnx.com/product/branches/7.0.0/trunk/hardware/flash/mtd-flash/amd/aMB_v2secsi.c $ $Rev: 810607 $")
#endif
