                    ...
                    uint32_t offset)
{
	...
	if (DQ4 == NULL) {
		...
		if (DQ4 == NULL) {
			fprintf(stderr, "(devf  t%d::%s:%d) malloc(%zd) returned NULL\n",
...
__SRCVERSION("$URL: http://svn.ott.qnx.com/product/branches/7.0.0/trunk/hardware/flash/mtd-flash/amd/aMB_v2erase.c $ $Rev: 810607 $")
#endif
