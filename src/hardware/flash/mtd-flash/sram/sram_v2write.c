	...
	for (i = 0; i < size; i++) {
		if (~dst[i] & src[i]) {
			fprintf(stderr, "(devf  t%d::%s:%d) Attempting to change 0's to 1's at offset %d in unit %d\n"
									"address 0x%"PRIxPTR"[0x%x] == 0x%.2x cannot be changed to 0x%.2x, "
									"bits 0x%.2x already 0\n",
									pthread_self(), __func__, __LINE__,
									(offset + i) % usize, (offset + i) / usize,
									(ptrdiff_t)dst, i, dst[i], src[i], ~dst[i] & src[i]);
...
__SRCVERSION("$URL: http://svn.ott.qnx.com/product/branches/7.0.0/trunk/hardware/flash/mtd-flash/sram/sram_v2write.c $ $Rev: 810607 $")
#endif
