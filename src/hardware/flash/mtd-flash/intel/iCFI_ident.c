                       ...
                       uint32_t offset)
{
	...
	if ((readmem(memory) & 0xff) == 0x20 || (readmem(memory) & 0xff) == 0x89)
	{
		if (readmem(memory + (0x13 * flashcfg.cfi_width)) == (F3S_BASETYPE)(0x02 * flashcfg.device_mult))
		{
			if(verbose > 2)
			{
				printf("(devf  t%d::%s:%d) cmd set = 0x%"PRIx64" != 0x%"PRIx64" (Intel), width %d device_mult 0x%"PRIx64"\n",
						pthread_self(), __func__, __LINE__,
...
__SRCVERSION("$URL: http://svn.ott.qnx.com/product/branches/7.0.0/trunk/hardware/flash/mtd-flash/intel/iCFI_ident.c $ $Rev: 810607 $")
#endif
