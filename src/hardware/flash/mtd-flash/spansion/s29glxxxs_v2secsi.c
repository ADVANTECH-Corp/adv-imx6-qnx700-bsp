			...
			int ops)
{
	...
	if (ops == LOCK_GET) {
		...
		f3s_s29glxxxs_flush((f3s_access_t *)access);
		return ret;
	}
...
int f3s_s29glxxxs_v2ssrop(f3s_dbase_t * dbase,
			f3s_access_t * access,
			uint32_t op,
			uint32_t offset,
			int32_t size,
...
__SRCVERSION("$URL: http://svn.ott.qnx.com/product/branches/7.0.0/trunk/hardware/flash/mtd-flash/spansion/s29glxxxs_v2secsi.c $ $Rev: 817897 $")
#endif
