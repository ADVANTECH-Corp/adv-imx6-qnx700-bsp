...
/* PLL3, PFD clocks */
static uint32_t mx6x_pll3_pfd_freq(int pfd)
{
	uint32_t    reg = in32(MX6X_ANATOP_BASE + MX6X_ANATOP_PFD_480);
	int         div;
...
uint32_t mx6x_get_enfc_clk(void)
{
    ...
    if ( (get_mx6_chip_type() == MX6_CHIP_TYPE_QUAD_OR_DUAL) &&
         (get_mx6_chip_rev()  == MX6_CHIP_REV_2_0)           )
    {
        clocksel = (cs2cdr & MX6X_CCM_CS2CDR_ENFC_CLK_SEL_MASK_V2_0) >> 
                   MX6X_CCM_CS2CDR_ENFC_CLK_SEL_OFFSET_V2_0;
    }
    else
    {
        clocksel = (cs2cdr & MX6X_CCM_CS2CDR_ENFC_CLK_SEL_MASK) >> 
    ...
    switch (clocksel) 
    {
        ...
        case 3:
            ...
            break;
        case 4:
            /* This option is only available in the iMx6 Quad Plus */
            clkroot = mx6x_pll3_pfd_freq(3);
            break;
    }
...
void mx6x_dump_clocks(void)
{
	if (debug_flag) {
		...
		kprintf("PLL2_PFD0     : %dHz\n", mx6x_pll2_pfd_freq(0));
		kprintf("PLL2_PFD1     : %dHz\n", mx6x_pll2_pfd_freq(1));
		kprintf("PLL2_PFD2     : %dHz\n", mx6x_pll2_pfd_freq(2));
		kprintf("PLL3_PFD0     : %dHz\n", mx6x_pll3_pfd_freq(0));
		kprintf("PLL3_PFD1     : %dHz\n", mx6x_pll3_pfd_freq(1));
		kprintf("PLL3_PFD2     : %dHz\n", mx6x_pll3_pfd_freq(2));
		kprintf("PLL3_PFD3     : %dHz\n", mx6x_pll3_pfd_freq(3));
		kprintf("\n");
	}
}
...
__SRCVERSION("$URL: http://svn.ott.qnx.com/product/branches/7.0.0/trunk/hardware/startup/boards/imx6x/mx6x_get_clocks.c $ $Rev: 783761 $")
#endif
