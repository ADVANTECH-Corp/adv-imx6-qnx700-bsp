...
int sdma_init(void)
{
	...
	out32( imx6x_sdma_base + MX6X_SDMA_CONFIG,
		in32( imx6x_sdma_base + MX6X_SDMA_CONFIG ) & ~CONFIG_CSM_MSK );
...
static int sdma_chnl_cfg(int ch_num)
{
	...
	cfg = in32( imx6x_sdma_base + MX6X_SDMA_CONFIG );
	if ( ( cfg & CONFIG_CSM_MSK ) == CONFIG_CSM_STATIC ) {
		cfg &= ~CONFIG_CSM_MSK;
		out32( imx6x_sdma_base + MX6X_SDMA_CONFIG, cfg | CONFIG_CSM_DYNAMIC );
	}
...
void sdma_copy(uint32_t dst, uint32_t src, uint32_t size)
{
	 ...
	 * Also ensure that SDMA transfer is >= MIN_SDMA_XFER_SIZE due to AP_2_AP RAM script requirement.
	 */
	unsigned leftover;

	leftover = size % MIN_SDMA_XFER_SIZE;
	size -= leftover;
...
__SRCVERSION("$URL: http://svn.ott.qnx.com/product/branches/7.0.0/trunk/hardware/startup/boards/imx6x/sdma_copy.c $ $Rev: 771519 $")
#endif
