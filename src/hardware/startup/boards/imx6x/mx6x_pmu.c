...
#define CPU_DQ_RECOMMENDED_SOC_VOLTAGE_MICROV	1250000		// 1.250 V
...
uint32_t pmu_set_voltage(uint32_t ldo_num, uint32_t req_voltage)
{
	...
	switch (ldo_num)
	{
		...
		case LDO_SOC:	// LDO_SOC = REG2
			...
			break;
		default:
			kprintf("%s: Unable to set LDO voltage - unknown LDO\n", __FUNCTION__);
			return EXIT_FAILURE;
	}
	...
	// need to add support for Internal Bypass mode which can be used for boards w/ external PMIC chips
	if (req_voltage < CPU_MIN_VOLTAGE_MICROV || req_voltage > CPU_MAX_VOLTAGE_MICROV)
	{
		kprintf("%s: Voltage value out of range, unable to set voltage\n", __FUNCTION__);
		return EXIT_FAILURE;
	}
...
__SRCVERSION("$URL: http://svn.ott.qnx.com/product/branches/7.0.0/trunk/hardware/startup/boards/imx6x/mx6x_pmu.c $ $Rev: 801719 $")
#endif
