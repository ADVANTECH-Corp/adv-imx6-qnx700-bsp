...
#define SIER_ROE1IE             (1<<11)
#define SIER_ROE0IE             (1<<10)
#define SIER_TUE1IE             (1<<9)
#define SIER_TUE0IE             (1<<8)
#define SIER_RFSIE              (1<<6)
#define SIER_TFSIE              (1<<7)
...
struct mx_card {
    ...
    unsigned int        *chmap_list;
    int                 chmap_list_size;
    dma_functions_t     sdmafuncs;

#define MAX_MIXEROPT 100
	char                mixeropts[MAX_MIXEROPT + 1];      /* Mixer Specific Options */
	int                 sample_rate;
	int                 sample_rate_min;
	int                 sample_rate_max;
	int                 sample_size;
...
__SRCVERSION("$URL: http://svn.ott.qnx.com/product/branches/7.0.0/trunk/hardware/deva/ctrl/mx/mxssi.h $ $Rev: 801725 $")
#endif
