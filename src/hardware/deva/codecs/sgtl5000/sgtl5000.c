 ...
 */
LOCAL int32_t
sgtl5000_audioman_refresh_set(MIXER_CONTEXT_T * hw_ctx, ado_dswitch_t * dswitch, snd_switch_t * cswitch,
                            void *instance_data)
{
    return (EOK);
}
...
LOCAL int32_t
sgtl5000_audioman_refresh_get(MIXER_CONTEXT_T * hw_ctx, ado_dswitch_t * dswitch, snd_switch_t * cswitch,
                            void *instance_data)
{
...
sgtl5000_reset (MIXER_CONTEXT_T * sgtl5000)
{
	...
	{
		/* 
		 * As per datasheet need to clear ANAPOWER_LINREG_SIMPLE_POWERUP bit prior to setting CHIP_LINREG_CTRL
		 * since VDDD is driven internally. Datasheet unclear about whether or not to clear ANAPOWER_STARTUP_POWERUP
	...
	/* weilun@adv - fixed no audio output on RSB-4410 begin */
	ana_power |= ANAPOWER_LINEOUT_POWERUP;	
	/* weilun@adv - fixed no audio output on RSB-4410 end */
...
build_sgtl5000_mixer (MIXER_CONTEXT_T * sgtl5000, ado_mixer_t * mixer)
{
	...
	if ((lineout_grp = ado_mixer_playback_group_create (mixer, SND_MIXER_LINE_OUT,
	...
	if ((dac_grp = ado_mixer_playback_group_create (mixer, SND_MIXER_MASTER_OUT,
	...
	if (ado_mixer_switch_new (mixer, "MIC BIAS", SND_SW_TYPE_BYTE,
		...
		return (-1);
	if (ado_mixer_switch_new(mixer, "Audioman Refresh", SND_SW_TYPE_BOOLEAN, 0, (void*)sgtl5000_audioman_refresh_get,
			(void *)sgtl5000_audioman_refresh_set, NULL, NULL) == NULL)
		return (-1);;
					...
					sgtl5000_config *sgtl5000conf)
{
	...
	/* weilun@adv move to codec_set_default_group */
#if 0
	...
	switch (channel)
	{
		case ADO_PCM_CHANNEL_PLAYBACK:
			ado_pcm_chn_mixer (pcm, ADO_PCM_CHANNEL_PLAYBACK, mixer,
							   ado_mixer_find_element (mixer, SND_MIXER_ETYPE_PLAYBACK1,SND_MIXER_PCM_OUT, index),
...
__SRCVERSION("$URL: http://svn.ott.qnx.com/product/branches/6.6.0/trunk/hardware/deva/codecs/sgtl5000/sgtl5000.c $ $Rev: 746215 $")
#endif
