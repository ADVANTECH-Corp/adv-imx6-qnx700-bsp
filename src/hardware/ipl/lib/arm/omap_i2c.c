...
/*
 * dbg = 0 : disable error message printing in read/write operation
 * This is useful if we call omap_i2c_read/write() with the purpose of polling
 * and it could have some known error messages
 */
	...
	while ((stat = in32(dev->base + I2C_STAT)) && ((stat & mask) != val)) {
		if (tries-- == 0) {
			...
			if ((in32(dev->base + I2C_STAT) & mask) != val) {
				if (omap_i2c_debug) {
					ser_putstr("omap_i2c_wait failed\n");
				}
				return -1;
			} else
				break;
...
int omap_i2c_read (omap_i2c_dev_t *dev, uint32_t reg, uint8_t alen, uint8_t* buffer, int len)
{
	...
	if (alen) {
		...
		if (-1 == omap_i2c_wait(dev, I2C_STAT_XRDY, I2C_STAT_XRDY)) {
			if (omap_i2c_debug) {
				ser_putstr("omap_i2c_read: XRDY not set\n");
			}
			return -1;
		}
		...
		if (-1 == omap_i2c_wait(dev, I2C_STAT_ARDY, I2C_STAT_ARDY)) {
			if (omap_i2c_debug) {
				ser_putstr("omap_i2c_read: ARDY not ready\n");
			}
			return -1;
		}
	}
	...
	if (-1 == omap_i2c_wait_fifo(dev, I2C_BUFSTAT_TXSTAT)) {
		if (omap_i2c_debug) {
			ser_putstr("omap_i2c_read: FIFO not ready\n");
		}
		return -1;
	}
	...
	if (-1 == omap_i2c_wait(dev, I2C_STAT_BB, I2C_STAT_BB))  {
		if (omap_i2c_debug) {
			ser_putstr("omap_i2c_read: wait busy\n");
		}
		return -1;
	}
	...
	for (i = 0; i < len; i++) {
		...
		if (-1 == omap_i2c_wait_fifo(dev, I2C_BUFSTAT_RXSTAT)) {
			if (omap_i2c_debug) {
				ser_putstr("omap_i2c_read: receive buffer not ready\n");
			}
			return -1;
		}
		...
		if (stat & I2C_STAT_ROVR) {
			out32 (dev->base + I2C_CON, 0);
			if (omap_i2c_debug) {
				ser_putstr("omap_i2c_read: rx overrun!\n");
			}
			return -1;
		}
	...
	if (-1 == omap_i2c_wait(dev, I2C_STAT_ARDY, I2C_STAT_ARDY)) {
		if (omap_i2c_debug) {
			ser_putstr("omap_i2c_read: register access not ready\n");
		}
		return -1;
	}
...
int omap_i2c_write(omap_i2c_dev_t *dev, uint8_t *buf, int len)
{
	...
	if (-1 == omap_i2c_wait(dev, 0, I2C_STAT_BB)) {
		if (omap_i2c_debug) {
			ser_putstr("omap_i2c_write: wait busy\n");
		}
		return -1;
	}
	...
	if (-1 == omap_i2c_wait(dev, I2C_STAT_XRDY, I2C_STAT_XRDY)) {
		if (omap_i2c_debug) {
			ser_putstr("omap_i2c_write: XRDY not set\n");
		}
		return -1;
	}
	...
	if (-1 == omap_i2c_wait(dev, I2C_STAT_ARDY, I2C_STAT_ARDY)) {
		if (omap_i2c_debug) {
			ser_putstr("omap_i2c_write: ARDY not set \n");
		}
		return -1;
	}
...
__SRCVERSION("$URL: http://svn.ott.qnx.com/product/branches/7.0.0/trunk/hardware/ipl/lib/arm/omap_i2c.c $ $Rev: 798438 $")
#endif
