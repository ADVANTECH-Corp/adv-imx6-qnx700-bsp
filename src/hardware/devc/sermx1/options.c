...
options(int argc, char *argv[])
{
    ...
    while (optind < argc) {
        ...
        /* Translate the TX FIFO trig level for DMA mode.
         * DMA TX watermark is in free space, but TX FIFO threadhold is in bytes used.
         * Max TX FIFO trig level is 32, Min is 2.
         * */
#if USE_DMA
        if( devinit.usedma ) {
            tx_fifo = FIFO_SIZE - tx_fifo + MIN_TX_FIFO;
        }
#endif
...
__SRCVERSION("$URL: http://svn.ott.qnx.com/product/branches/7.0.0/trunk/hardware/devc/sermx1/options.c $ $Rev: 813794 $")
#endif
