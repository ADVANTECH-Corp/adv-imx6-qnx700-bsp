...
mx6q_parse_options (mx6q_dev_t *mx6q, char *options, nic_config_t *cfg, struct mx6q_arg *mx6q_arg)
{
   ...
   unsigned hwi_off = hwi_find_device("fec", mx6q_arg->idx);
    ...
    while (options && *options != '\0') {
        ...
        case 5:
            if (mx6q && value) {
                mx6q->brmast = strtoul(value, 0, 0);
            }
            break;
#ifdef MX6XSLX
        case 6:
            if (mx6q && value) {
                mx6q->rx_frame = strtol(value, 0, 0);
            }
            break;

        case 7:
            if (mx6q && value) {
                mx6q->rx_delay = strtol(value, 0, 0);
            }
            break;

        case 8:
            if (mx6q && value) {
                mx6q->tx_frame = strtol(value, 0, 0);
            }
            break;

        case 9:
            if (mx6q && value) {
                mx6q->tx_delay = strtol(value, 0, 0);
            }
            break;
#endif
        default:
...
mx6q_attach(struct device *parent, struct device *self, void *aux)
{
    ...
    if (cfg->verbose) {
        log(LOG_ERR, "%s(): IF %d: Base register 0x%08" PRIXPTR,
    ...
    if ((cfg->media_rate != -1) || (cfg->duplex != -1)) {
        ...
        default:
            log(LOG_ERR, "%s(): invalid speed: %d", __FUNCTION__,
		cfg->media_rate/1000);
            mx6q_destroy(mx6q, 1);
            return EINVAL;
            break;
    ...
    if(mx6q->phy_base != 0 && mx6q->phy_base != mx6q->iobase) {
    ...
    if ((mx6q->tx_bd = mmap (NULL, sizeof (mpc_bd_t) * mx6q->num_tx_descriptors * NUM_TX_QUEUES,
        PROT_READ | PROT_WRITE | PROT_NOCACHE , MAP_ANON | MAP_PHYS | MAP_SHARED, NOFD, 0)) == MAP_FAILED) {
        rc = errno;
    ...
    memset(&mx6q->rtc_spinner, 0, sizeof(mx6q->rtc_spinner));
    ...
    *(base + MX6Q_IEVENT) = 0xffffffff;
    ...
    if (mx6q->iid_1588 == -1) {
        if ((rc = InterruptAttach_r(mx6q->cfg.irq[1], mx6q_1588_isr,
            mx6q, sizeof(*mx6q), _NTO_INTR_FLAGS_TRK_MSK)) < 0) {
            rc = -rc;
            log(LOG_ERR, "%s(): InterruptAttach_r(1588) failed: %d", __FUNCTION__, rc);
    ...
    // Attach to hardware interrupts
    if (mx6q->iid_rx == -1) {
        if ((rc = InterruptAttach_r(mx6q->cfg.irq[1], mx6q_isr,
            mx6q, sizeof(*mx6q), _NTO_INTR_FLAGS_TRK_MSK)) < 0) {
            rc = -rc;
            log(LOG_ERR, "%s(): InterruptAttach_r(rx) failed: %d", __FUNCTION__, rc);
            mx6q_destroy(mx6q, 12);
    ...
    if (mx6q->iid_tx == -1) {
        if ((rc = InterruptAttach_r(mx6q->cfg.irq[2], mx6q_isr,
            mx6q, sizeof(*mx6q), _NTO_INTR_FLAGS_TRK_MSK)) < 0) {
            rc = -rc;
            log(LOG_ERR, "%s(): InterruptAttach_r(tx) failed: %d", __FUNCTION__, rc);
            mx6q_destroy(mx6q, 12);
            return rc;
        }
        mx6q->iid_tx = rc;
    ...
    if (mx6q->iid_err == -1) {
        if ((rc = InterruptAttach_r(mx6q->cfg.irq[3], mx6q_isr,
            mx6q, sizeof(*mx6q), _NTO_INTR_FLAGS_TRK_MSK)) < 0) {
            rc = -rc;
            log(LOG_ERR, "%s(): InterruptAttach_r(err) failed: %d", __FUNCTION__, rc);
            mx6q_destroy(mx6q, 12);
            return rc;
        }
        mx6q->iid_err = rc;
    ...
    *(base + MX6Q_IMASK) = IMASK_RXF2EN | IMASK_RXF1EN | IMASK_RFIEN |
    ...
    if (mx6q->tx_frame || mx6q->tx_delay) {
        if (!mx6q->tx_frame || !mx6q->tx_delay) {
            log(LOG_ERR, "%s(): must set BOTH tx_delay %d and tx_frame %d",
    ...
    *(base + MX6SLX_TXIC2) = txic_val;
    ...
    if (mx6q->rx_frame || mx6q->rx_delay) {
        if (!mx6q->rx_frame || !mx6q->rx_delay) {
            log(LOG_ERR, "%s(): must set BOTH rx_delay %d and rx_frame %d",
...
mx6q_init (struct ifnet *ifp)
{
	...
	*(base + MX6Q_R_DES_ACTIVE) = R_DES_ACTIVE;
	...
	if ((*(base + MX6Q_QOS_SCHEME) & QOS_SCHEME_RX_FLUSH0) != 0) {
	    /* Rx class match is setup enable Rx on class 1 & 2 */
...
mx6q_stop(struct ifnet *ifp, int disable)
{
    ...
    *(base + MX6Q_ECNTRL) |= ECNTRL_SLEEP;
    ...
    if (i >= STOP_TIMEOUT) {
	log(LOG_ERR, "%s(): Failed to gracefully stop", __FUNCTION__);
    }
...
mx6q_detect(void *dll_hdl, struct _iopkt_self *iopkt, char *options)
{
	...
	if (opt == 0) {
	    if (value == NULL) {
		log(LOG_INFO, "%s: Ignoring deviceindex without value",
		    __FUNCTION__);
	    } else {
    ...
    // Attach all mx6x ethernet controllers
    for (i = 0; ; i++) {
	...
	hwi_off = hwi_find_device("fec", mx6q_arg.idx);
	if (hwi_off == HWI_NULL_OFF) {
	    break;
	}
    ...
    if (i == 0) {
        log(LOG_ERR, "%s(): could not attach any devices", __FUNCTION__);
        return ENODEV;
    }

    log(LOG_INFO, "%s(): attached %u devices", __FUNCTION__, i);
    return 0;
}

#if defined(__QNXNTO__) && defined(__USESRCVERSION)
#include <sys/srcversion.h>
__SRCVERSION("$URL: http://svn.ott.qnx.com/product/branches/7.0.0/trunk/hardware/devnp/mx6x/detect.c $ $Rev: 819741 $")
#endif
