 ...
 *
 */
void mx6q_ptp_start (mx6q_dev_t *mx6q)
{
    volatile uint32_t *base = mx6q->reg;
    ...
    *(base + MX6Q_TIMER_CTRLR) |= MX6Q_TIMER_CTRL_EN;
    ...
    /*
     * Setup Timer Compare 0 to generate half-rollover events
     * N.B. Load compare value before enabling and then queue up
     * next compare value to be loaded when first compare happens.
     */
...
void mx6q_ptp_add_rx_timestamp (mx6q_dev_t *mx6q, ptpv2hdr_t *ph, mpc_bd_t *bd)
{
    ...
    /*
     * Handle rollovers of the timer and async updates of the
     * seconds stored in software.
     */
    if ((rx_ts[rx_ts_cnt].ts.nsec > 750000000) &&
	(mx6q->rtc_half == 0)) {
	/*
	 * Captured from hardware before the second but by the time
	 * we read the software seconds then it had incremented.
	 */
	rx_ts[rx_ts_cnt].ts.sec--;
    } else if ((rx_ts[rx_ts_cnt].ts.nsec < 250000000) &&
	       (mx6q->rtc_half == 1)) {
	/*
	 * Captured from hardware after the second but the software
	 * seconds have not incremented yet.
	 */
	rx_ts[rx_ts_cnt].ts.sec++;
    }
...
void mx6q_ptp_add_tx_timestamp (mx6q_dev_t *mx6q, ptpv2hdr_t *ph, mpc_bd_t *bd)
{
     ...
     * seconds stored in software.
     */
    if ((tx_ts[tx_ts_cnt].ts.nsec > 750000000) &&
	(mx6q->rtc_half == 0)) {
	/*
	 * Captured from hardware before the second but by the time
	 * we read the software seconds then it had incremented.
	 */
	tx_ts[tx_ts_cnt].ts.sec--;
    } else if ((tx_ts[tx_ts_cnt].ts.nsec < 250000000) &&
	       (mx6q->rtc_half == 1)) {
	/*
	 * Captured from hardware after the second but the software
	 * seconds have not incremented yet.
...
int mx6q_ptp_get_tx_timestamp (mx6q_dev_t *mx6q, ptp_extts_t *ts)
{
    ...
    /* Reap the descriptors to pick up any new timestamps */
    ifp = &mx6q->ecom.ec_if;
    NW_SIGLOCK(&ifp->if_snd_ex, mx6q->iopkt);
    for (i = 0; i < NUM_TX_QUEUES; i++) {
	mx6q_transmit_complete(mx6q, i);
    }
...
void mx6q_ptp_get_cnt (mx6q_dev_t *mx6q, ptp_time_t *cnt)
{
	 ...
	 * seconds stored in software.
	 */
	if ((cnt->nsec > 750000000) && (mx6q->rtc_half == 0)) {
	    /*
	     * Captured from hardware before the second but by the time
	     * we read the software seconds then it had incremented.
	     */
	    cnt->sec--;
	} else if ((cnt->nsec < 250000000) && (mx6q->rtc_half == 1)) {
	    /*
	     * Captured from hardware after the second but the software
	     * seconds have not incremented yet.
	     */
...
__SRCVERSION("$URL: http://svn.ott.qnx.com/product/branches/7.0.0/trunk/hardware/devnp/mx6x/ptp.c $ $Rev: 805227 $")
#endif
